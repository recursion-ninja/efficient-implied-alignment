...
instance Ord FilePoint where
    ...
    compare lhs rhs = case comparing taxaCount lhs rhs of
...
main = do
    ...
    let strLens   = NE.fromList . sort $ (truncate . (toRational coefficient *) &&& id) <$> stringLengths opts
        ...
        taxaNumPad =
            let n = length $ show numFoundTaxa in \x -> let s = show x in replicate (n - length s) ' ' <> s
        strLenPad =
            let n = length $ show coefficient in \x -> let s = show x in replicate (n - length s) ' ' <> s
...
gatherReceiptsByTaxaCount =
    let f
            :: DataFileReceipt
            -> Map (Word, FilePath) (NonEmpty DataFileReceipt)
            -> Map (Word, FilePath) (NonEmpty DataFileReceipt)
...
getFilePoints filePath taxaCounts strFractions = do
    ...
    where
        reportMissingFiles :: Foldable f => f FilePath -> IO a
        ...
        collectErrors es = case partitionEithers $ toList es of
        ...
        getFilePoint taxaSize strLength = do
            let onlyFileName = getFileName filePath
                ...
                dataFilePath =
                    replicationDataDirectory </> onlyFileName <.> show taxaSize <.> show strLength <.> "fasta"
...
parseRuntimes str = case parse traversalRuntimes "STDOUT" str of
...
parseCPUTime = fmap fromPicoseconds . choice $ try <$> [days, hours, mins, secs, msecs, μsecs, nsecs, psecs]
    where
        ...
        mins =
            (+) <$> ((minute *) <$> (decimal <* single 'm')) <*> ((second *) <$> (decimal <* string "sec"))
        secs = (+) <$> ((second *) <$> (decimal <* single '.')) <*> ((mSecond *) <$> (decimal <* string "s"))
        msecs =
            (+) <$> ((mSecond *) <$> (decimal <* single '.')) <*> ((μSecond *) <$> (decimal <* string "ms"))
        μsecs =
            (+) <$> ((μSecond *) <$> (decimal <* single '.')) <*> ((nSecond *) <$> (decimal <* string "μs"))
...
binaryDirectory, replicationDirectory, replicationDataDirectory, replicationTaxaDirectory, replicationTreeDirectory, replicationOutputDirectory, replicationImageDirectory
    :: FilePath
binaryDirectory = "./bin/"
    ...
    let onlyFileName = getFileName filePath
        ...
        decimalVer =
            case normalizeDecimal <$> (eitherFromRational fraction :: Either String (DecimalRaw Natural)) of
                ...
                Right dec -> show dec
        commandStr = unwords [binFilePath, dataFilePath, show taxaSize, decimalVer, taxaFilePath]
...
generateTruncatedTreeFile taxaNumPadder filePath counter (taxaSize, taxaFilePath) receipts = do
    ...
    let onlyFileName = getFileName filePath
        ...
        commandStr   = unwords [binFilePath, "delete", treeFilePath, taxaFilePath <.> "deleted"]
...
makeProcessFromCommand commandStr = CreateProcess
