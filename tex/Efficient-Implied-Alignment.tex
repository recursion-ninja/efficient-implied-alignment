...
\begin{abstractbox}
	...
	\begin{abstract}
		Given a binary tree $\mathcal{T}$ of $n$ leaves, each leaf labelled by a string of length at most $k$, and a binary string alignment function $\otimes$, an implied alignment can be generated to describe the alignment of a dynamic homology for $\mathcal{T}$.
		...
		Previous descriptions of the implied alignment algorithm suggest a technique of ``back-propagation'' with time complexity $\mathcal{O}(k^2 * n^2)$ time.
		Here we describe an implied alignment algorithm with a complexity of \textsc{$\mathcal{O}(k * n^2)$}.
		For well-behaved data, such as molecular sequences, the best-case complexity approaches \textsc{$\Omega(k * n)$}.
...
Although originally designed for alignment-free phylogenetic analysis (dynamic homology, \citealp{Wheeler2001}), the procedure was used as a stand-alone multiple sequence alignment (MSA) tool by \cite{WhitingAetal2006} in their analysis of skink systematics.
...
The algorithm uses the repeated application of a pairwise string alignment function (the method is agnostic) to perform an efficient multiple string alignment for a given binary tree whose leaves are labelled by strings. I.e., the tree describes the relationship of those strings.
The more similar the initial leaf labellings the better the algorithm performs.
...
Let $\otimes : \Sigma^{*}_{\Gamma} \times \Sigma^{*}_{\Gamma} \rightarrow \left(\NonNegReals,\; \Sigma^{*}_{\Gamma}\right)$ be a heuristic function that returns a nonnegative alignment cost and alignment context.
...
First, we perform a post-order traversal---i.e. a node is visited after its children---from the leaves to the root, using $\otimes$ to produce preliminary contexts for each node. 
Second, we perform a pre-order traversal, from the root to the leaves, aligning each preliminary context with its parent to produce a final alignment on each node.
...
We are able to improve this by, during the post-order pass, tagging each element of a string on a node $n_x$ with information that notates on which subtree of $n_x$ that element originated.
...
These tagged pairwise alignments will be consumed on the subsequent pre-order traversal of $\mathcal{T}$ when merging alignment contexts.
...
Note that $\sigma$ can be a more complex metric than those presented here, for instance metric with affine or logarithmic affine gap costs, and be compatible with the IA algorithm. For usage of $\sigma$, see Algorithms \ref{Alg:pairwiseAlignment}, \ref{Alg:matrixDefinition}, and \ref{Alg:matrixTraceback}.
...
These preliminary contexts will be consumed to assign a final string alignment in the subsequent pre-order traversal of the tree.
...
For each non-root node, $n_c$, we first determine whether $n_c$ is the left or right child of its parent.
This is required because deletion events originate from alignments of the left subtree and insertion events originate from alignments of the right subtree, and we must use this information when deriving the final alignment of $n_c$.

The final alignment of the parent of $n_c$, $n_p$, will necessarily be of greater than or equal length to $n_p$'s preliminary alignment, because $n_p$'s final alignment contains all the information from the contexts of $n_p$'s subtrees as well as the information from the rest of the tree, that is, the contexts of all of the subtrees of every ancestor node to $n_p$.
The preliminary context of $n_p$ is also greater than or equal to the preliminary context of $n_c$, due to the $n_p$'s context containing all information from $n_c$'s context, plus the addition of $n_c$'s sister subtree.
The resulting value assigned to $n_c$'s final alignment will have the same length as the final alignment assigned to $n_p$.
...
The final alignment for $n_c$ is derived by performing a ``sliding zip'' over $n_p$'s final alignment, $n_p$'s preliminary context, and $n_c$'s preliminary context.
$n_p$'s final alignment is used as the basis of the zip. The inputs to this alignment are the preliminary contexts of $n_p$ and $n_c$ and $n_p$'s final alignment.
At each step of the ``sliding zip,'' one element of $n_p$'s final alignment will be consumed and one element of $n_c$'s final alignment will be defined.
Additionally, at each step of the zip, one of: an element from $n_p$'s preliminary context, elements from both $n_p$'s and $n_c$'s preliminary contexts, or no elements from either node's context, will be consumed.
Finally, we define an element of $n_c$'s final alignment to be either an element from $n_c$'s preliminary context or a gap.
The process is called a ``sliding zip'' because, due to the varying lengths of the three inputs, the elements of $n_p$'s and $n_c$'s preliminary contexts do not have an immediately apparent index with which they correspond to $n_p$'s final alignment, which is used as the basis of the zip.
Rather, the elements of $n_p$'s and $n_c$'s preliminary contexts ``slide'' through the zipping process, and their corresponding indices with the $n_p$'s final assignment is deduced dynamically.
The logic applied in the ``sliding zip'' is to propagate gaps from the tree above $n_c$ down to $n_c$, and when not dealing with a gap propagated from an ancestor node to $n_c$, to align the non-gap elements or introduce a new gap to be propagated.
...
There are five cases determining the derivation of each index of $n_c$'s final alignment. The cases are presented in the pseudocode of Algorithm \ref{Alg:deriveAlignment}, in Figure \ref{Fig:Examplepre-order}, and described below.

\begin{itemize}

  \item Case 0: When the sliding zip has consumed all elements of the $n_c$'s preliminary context, then the next element of $n_c$'s final alignment is ``\texttt{GAPPED}.''
  Because we only define the next element of $n_c$'s final alignment to be either an element from $n_c$'s preliminary context or a gap, the latter is the only choice.

  \item Case 1: When the element of $n_p$'s final alignment is ``\texttt{ALIGN},'' then we consume the next elements of both $n_p$'s and $n_c$'s preliminary contexts and the next element of $n_c$'s final alignment is $n_p$'s consumed preliminary context element.
  Because $n_p$'s final alignment element was marked as an alignment event, we know that $n_c$ was aligned with its sister subtree at this index, and that $n_c$'s preliminary context element is the correct element for this index of the alignment.
  
  \item Case 2: When the both $n_p$'s final alignment element and $n_p$'s preliminary context element are ``\texttt{DELETE},'' and $n_c$ is the \textit{left} child of its parent, then we consume the next element of each of $n_p$'s and $n_c$'s preliminary contexts and the next element of $n_c$'s final alignment is $n_c$'s consumed preliminary context element.
  Because deletion events originate from the left subtree of a node, and $n_c$ is the left child of $n_p$, $n_c$'s preliminary context element is the correct element for this index of the alignment.
  If the same deletion event was encountered but $n_c$ was the right child of $n_p$, then $n_c$'s preliminary context element would not be the correct element for this index of the alignment, because deletion events originate from the left subtree and the deletion event was encountered in the right subtree.
  ...
  This is caught by Case 4.
  
  \item Case 3: When the both $n_p$'s final alignment element and $n_p$'s preliminary context element are ``\texttt{INSERT},'' and $n_c$ is the \textit{right} child of $n_p$, we consume the next element of both $n_p$'s and $n_c$'s preliminary contexts and assign to the next element of $n_c$'s final alignment $n_c$'s consumed preliminary context element.
  Because insertion events originate from the right subtree of a node, and $n_c$ is the right child of $n_p$, $n_c$'s preliminary context element is the correct element for this index of the alignment.
  If the same insertion event was encountered but $n_c$ was the left child of $n_p$, then $n_c$'s preliminary context element would not be the correct element for this index of the alignment, because insertion events originate from the right subtree and the insertion event was encountered in the left subtree.
  In the case that a insertion event is encountered in a left subtree, we introduce a new gap into all the alignments of the subtree at this index to account for the aligned element in $n_c$'s sister subtree.
  This is caught by Case 4.
  
  \item Case 4: When none of the conditions for Case 0, 1, 2 or 3 hold, then we consume the next element of $n_p$'s preliminary context and the next element of $n_c$'s final alignment is ``\texttt{GAPPED}.''
...
\begin{backmatter}
	...
	\begin{table}[!htbp] \centering 
		...
		\begin{tabular}{@{\extracolsep{5pt}}lD{.}{.}{-3} D{.}{.}{-3} D{.}{.}{-3} D{.}{.}{-3} } 
			...
			Adjusted R$^{2}$ & \multicolumn{1}{c}{0.958} & \multicolumn{1}{c}{0.959} & \multicolumn{1}{c}{0.986} & \multicolumn{1}{c}{0.996} \\ 
			\hline 
			\hline \\[-1.8ex] 
		\end{tabular} 
	\end{table}
...
%	\end{figure}
	...
	\begin{algorithm}
		...
		\begin{algorithmic}[1]
			...
			\Else
			\State \Return $\Call{matrixTraceback}{\sigma, \textit{rhs}, \textit{lhs}}$ \Comment{See Algorithm \ref{Alg:matrixTraceback}}
			\EndIf
			\EndFunction
		\end{algorithmic}
	\end{algorithm}
	...
	\begin{algorithm}
		\caption{Post-order Traversal}\label{Alg:post-order}
		\begin{algorithmic}[1]
