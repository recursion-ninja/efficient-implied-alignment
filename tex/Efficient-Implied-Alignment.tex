...
\begin{abstractbox}
	...
	\begin{abstract}
		...
		This is done by first decorating each node of $\mathcal{T}$ with an alignment context using $\otimes$, in a post-order traversal, then, during a subsequent pre-order traversal, inferring on which edges insertion and deletion events occurred using those internal node decorations.
		\section*{Results}
...
Similar MSA approaches also based in probabilistic analysis have been described e.g by \cite{Loytynoja2005} and \cite{Paten2008}, and implemented in PRANK \citep{PRANK}. Whiting et al. found that IA was superior (in terms of tree optimality score) to other MSA methods in both parsimony and likelihood analyses.
...
%% Apologies if the following note is irrelevant: I'm writing after having waited several days since reading the first half of the paper.

%% I don't think you ever actually define "preliminary context", at least in a rigorous way. Also, you say it's getting zipped together to create a "final 
%% alignment, which implies to me that "preliminary context" is actually some kind of alignment or at least sequence. From my last reading of this paper
%% I thought it also included the cost, but you seem to have broken that out here, right? Or maybe I never understood. This paper is much more clear.
%% Oh, actually I see later that you at least imply the meaning of "preliminary context" later by saying it's composed of the alphabet Sigma{lambda}.
%% I'd still like that to be explicit. 
...
Because elements tagged \textbf{\texttt{GAPPED}} originated from neither subtree, \textbf{\texttt{GAPPED}} those elements cannot be determined during the post-order, only derived during the pre-order.
...
This consistency in assignment ensures the commutativity of $\otimes$, which is necessary to enforce consistency of the IA algorithm, i.e. that it provides the same alignment results for isomorphic tree labelings, guaranteeing label invariance.
 
We now apply a memoized update procedure \citep{cormen2001}, a common element of dynamic programming algorithms such as Needleman-Wunsch alignment.
...
This additional tagging detail is the key difference between previous alignment methods and the method presented in this paper.
...
For clarity, while this example function is presented as a modification of the well understood Needleman-Wunsch algorithm (by adding explicit memoization), this tagging approach can be incorporated into more sophisticated pairwise string alignment algorithms.
...
The operator $\sigma \,:\, \NEPowerset(\Sigma) \times \NEPowerset(\Sigma) \rightarrow \left(\NonNegReals,\, \NEPowerset(\Sigma) \right)$ presented in  ref{Alg:pairwiseAlignment}, ref{Alg:matrixDefinition}, and ref{Alg:matrixTraceback} represents a metric for determining the transition cost between symbols in $\NEPowerset (\Sigma)$.
%% I noticed that none of the \ensures in your algorithms are printing. If thatÕs a mistake, youÕll need to fix my edit on l. 296 because \sigma is going 
%% to appear in nearly every algorithm definition. /sigma
The metrics used in our data sets can be found in Table \ref{Tab:Metrics}. The metrics presented in Table \ref{Tab:Metrics} show the transition costs between elements of $\Sigma$.
...
%% See here the note above re contexts vs. alignments vs. preliminary strings, and the (possible?) conflation of the latter two in this explanation.
...
%% You're not assigning the preliminary context to the final alignment? Where's the root's final alignment coming from?
...
The preliminary context of $v_p$ is also of greater than or equal length to the preliminary context of $v_c$, due to $v_p$'s context containing all information from $v_c$'s context, plus the addition of $v_c$'s sister subtree.
...
%% Is that argument clear? I really had to think about it. I think you're saying that the final length of the context of each node is necessarily >= to 
%% the preliminary contexts of each of its parents, and the final contexts of those parents are necessarily of >= length of all of their child nodes, 
%% and this parent is a child node of its parent, so they must both be equal as both can't be greater than the other?
%% And containing more information guarantees a greater length, rather than just adding more information to each element of that length? 
%% Oh, wait. I think that the second question is actually answered in the next para.
...
%% Can you talk a little more about that diagram? What are the meanings of the various colors? Why are some elements unlabeled, that is, not 
%% B, G, L, R? I wish you could present the cases in order from 0 to 5, but I get that you didn't.
%% Also, you present some cases as having two choices but you don't make clear in the diagram which choice you're making. E.g. in case three
%% is the child a left or a right child?

There are five cases determining the derivation of each index of $v_c$'s final alignment. The cases are both presented in the pseudocode of Algorithm \ref{Alg:deriveAlignment}, in Figure \ref{Fig:Examplepre-order} and described below.
...
\begin{itemize}
 ...
 %% Are there not actually Cases 3a and 3b here?
 ...
 %% Could you say "conversely to Case 3" or "similarly to Case 3 but with dealing with right-tagged elements of $v_p$" or something here? 
...
%% I think this para goes somewhere else.
...
Additionally, the $k^2$ component in the previous implementation was due to using a Needleman-Wunsch string alignment between the current node and its parent node at each post-order and pre-order step.
By saving the requisite information on the nodes during the post-order traversal and then consuming this information with the ``sliding-zip'' technique, we eliminate one of these Needleman-Wunsch alignments at each internal node, as well as the back-propogation, which has a cost of $k$ at each internal node, $n$.
...
Hence, the worst case time complexity of the implied alignment algorithm is $\mathcal{O}(k * n^2)$ and occurs when the input strings are independent of each other.
...
The full leaf set of the tree was repeatedly halved to produce a data set of doubling leaf set sizes. %% halved but doubled? It could be more clear.
...
%% I think further discussion of associativity and its relevance here is warranted.
...
%% "is why" is too informal, but I couldn't come up with a good substitute, because I'm tired.
