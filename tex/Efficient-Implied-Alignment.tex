  ...
  \begin{algorithmic}[1]
    ...
    \Function{reverseContext}{$\textit{context}$}
      \Switch{$\textit{context}$}
        \Case{    $\texttt{ALIGN} \quad \textit{x} \;\; \textit{y} \;\;$}
          \Return $\texttt{ALIGN} \quad \textit{y} \;\; \textit{x} \;\;$
        \EndCase
        \Case{    $\texttt{DELETE} \;\; \textit{x} \;\;\quad$}
          \Return $\texttt{INSERT} \;\; \quad           \textit{x} \;\;$
        \EndCase
        \Case{    $\texttt{INSERT} \;\; \quad \textit{y} \;\;$}
          \Return $\texttt{DELETE} \;\;      \textit{y} \;\;$
        \EndCase
        \Case{    $\texttt{GAPPED} \;\; \quad \quad$}
          \Return $\texttt{GAPPED}$
        \EndCase
      \EndSwitch
    \EndFunction
  ...
  \begin{algorithmic}[1]
    \Function{deriveAlignment}{$\textit{isLeaf},\,  \textit{pAlignment},\, \textit{pContext},\, \textit{cContext}$}
      ...
      \For{$\textit{i} < \textit{len}$}
        \If    {$\textit{k} \geq \textit{ccLen}$}
          \State $\textit{result}_i \gets \texttt{GAPPED}$ \Comment{Case 0}
        \Else
