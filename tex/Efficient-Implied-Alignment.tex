...
As such, any individual IA is not guaranteed to be neither optimal nor unique, with potentially an exponential number of equally optimal implied alignments for any given binary tree.
...
One candidate function fitting the description of $\otimes$ can be defined similarly to the Needleman-Wunsch \citep{Needleman1970} algorithm for pairwise string alignment.
The algorithm is modified along same the lines that DO modified the dynamic programming technique of \cite{Sankoff2000}, with an additional step taken to produce the tagged elements in the output alignment.
...
Storing this information for each element of the pairwise alignment allows a more efficient generation of the subsequent multiple string alignment.
...
Affine gap models described by \cite{Gotoh1982} can be incorporated via the method of \cite{VaronandWheeler2012}.
...
If we were to use Ukkonen's method with the $\otimes$ described in Algorithm \ref{Alg:pairwiseAlignment}, the post-order traversal would run in $\mathcal{O}(k * \log  k * n)$ average case time and space.
