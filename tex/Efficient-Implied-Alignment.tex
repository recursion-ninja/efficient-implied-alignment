...
The major difference is the additional stored information allows us to decorate the final assignments in $\mathcal{O}(n*k^{2-c})$ instead of $\mathcal{O}(n^2*k^2)$ time. 
The aforementioned $n^2$ component occured in previous implementations using a ``back-propogation'' technique, where at each pre-order step, each new gap found in the alignment was retroactively applied to every alignment derived in a previous pre-order step.
Additionally, the aforementioned $k^2$ component occured in previous implementations using a Needleman-Wunsh string alignment between the parental node and the current node in each preorder step.
By performing the decorating all the requisite information on the nodes in the postorder traversal, then consuming this information with the ``sliding-zip'' technique, we replace these computationally expensive operations with a much more efficient algorithm.
