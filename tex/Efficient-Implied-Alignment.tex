...
\begin{abstractbox}
	...
	\begin{abstract}
		Given a binary tree $\mathcal{T}$ of $n$ leaves, each leaf containing a string of length at most $k$, and a binary string alignment function $\otimes$, an implied alignment can be generated to describe the alignment of a dynamic homology for $\mathcal{T}$.
...
It is worth explaining the motivation of the constructions defined above. 
...
The results of $\odot$ cannot preserve all information from it's inputs, but the construction of $\otimes$ can do so.
Figure ?? demonstrates how $\odot$ can produce a result with ambiguous relationship to it's inputs, but $\otimes$ preserves this information.
The preservation of result's relationship to it's input is require for the algorithmic improvements presented below.
...
We now apply a ``wave-front'' update procedure \citep{cormen2001}, which is very similar to the Needleman-Wunsch procedure to align two strings.
