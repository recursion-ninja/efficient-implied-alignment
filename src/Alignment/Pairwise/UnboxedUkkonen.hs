...
import Alignment.Pairwise.Internal (Direction(..), insertGaps, measureAndUngapCharacters, measureCharacters)
    ...
    where
        ...
        coefficient = minimum $ indelCost <$> nonGapElements
            where
                ...
                indelCost i = min (cost (bit i) gap) (cost gap (bit i))
...
createUkkonenMethodMatrix gap minimumIndelCost gapsPresentInInputs tcm lesserLeft longerTop = finalMatrix
    ...
    -- General values that need to be in scope for the recursive computations.
        ...
        quasiDiagonalWidth = toEnum $ differenceInLength + 1
            where differenceInLength = longerLen - lesserLen
                ...
                then pure False
                    ...
                    let threshold
                            | -- The threshold value must be non-negative
                              quasiDiagonalWidth + offset <= gapsPresentInInputs
                            = 0
                            | otherwise
                            = minimumIndelCost * (quasiDiagonalWidth + offset - gapsPresentInInputs)
    ...
    -- Note: "offset" cannot cause "width" to exceed "cols"
        ...
        fullMatrix                                            = do
          ...
          ---------------------------------------
            ...
            ---------------------------------------
            let (write, internalCell, leftColumn, leftBoundary, rightBoundary, rightColumn) =
            ...
            let writeRow i =
                  -- Precomute some values that will be used for the whole row
                    ...
                    in  do -- Write to the first cell of the Ukkonen band
                        ...
                        for_ [start + 1 .. stop - 1]
                            $ \j -> internalCell leftElement insertCost i j >>= write (i, j)
            ...
            for_ [1 .. min (cols - 1) (width - offset - 1)] $ \j ->
                ...
                in  do
                    firstPrevCost <- M.unsafeRead mCost (0, j - 1)
            ...
            let extendRow i =
                  -- Precomute some values that will be used for the whole row
                    let start0      = max 0 $ i - offset
                        ...
                        recomputeUntilSame j =
                            snd <$> iterateUntilM continueRecomputing computeCell' (False, j)
                    in  do -- First, we fill in 0 or more cells of the left region of
                        ...
                        when (s0 > e0 && toEnum i > po)
                            $   recomputeRange leftElement insertCost i (e0 + 1) s0
                            >>= writeSTRef t0'
                        t0 <- readSTRef t0'
                        ...
                        when (i < rows - fromEnum po) $ lastCell leftElement insertCost i stop >>= write
                            (i, stop)
        ...
        fullMatrix = runST $ do
            ...
            for_ [1 .. cols - 1] $ \j ->
                let topElement    = symbolAlignmentMedian $ topChar ! (j - 1)
                    firstCellCost = cost gap topElement
                in  do
                    firstPrevCost <- V.unsafeRead vOne (j - 1)
            ...
            for_ [1 .. rows - 1] $ \i ->
                ...
                in  do
                    firstPrevCost <- V.unsafeRead prev 0
                    ...
                    for_ [1 .. cols - 1] $ \j ->
                        let topElement             = symbolAlignmentMedian $ topChar ! (j - 1)
                            ...
                            insertCost             = cost gap leftElement
                        in  do
                            diagCost <- V.unsafeRead prev $ j - 1
...
directOptimization gap overlapλ matrixFunction char1 char2 =
    let (swapped, gapsLesser, gapsLonger, shorterChar, longerChar) =
...
traceback gap overlapFunction alignMatrix lesserChar longerChar = force alignmentContext
    where
        ...
        dlistToDynamic :: Foldable t => t a -> NEV.Vector a
        ...
        go !p@(~(!i, !j))
            ...
            = let
                  ...
                  (# !row', !col', !localContext #) = case directionArrow of
                    LeftArrow ->
                        let j' = j - 1
...
getMinimalResult
    :: Foldable f => SymbolAmbiguityGroup -> SymbolAmbiguityGroup -> f (Word, Direction) -> (Word, Direction)
...
ukkonenConstants tcm lesserLeft longerTop o = (offset, cost, rows, cols, width, quasiDiagonalWidth)
    where
    -- Note: "offset" cannot cause "width + quasiDiagonalWidth" to exceed "2 * cols"
        ...
        quasiDiagonalWidth = differenceInLength + 1 where differenceInLength = longerLen - lesserLen


cellDefinitions
    :: (Indexable f, Key f ~ Int, PrimMonad s)
    => SymbolAmbiguityGroup
    -> f SymbolContext
