...
import Alignment.Pairwise.Ukkonen.Ribbon qualified as Ribbon
...
ukkonenDO alphabet overlapFunction lhs rhs
    ...
    | otherwise = directOptimization
        gapGroup
        overlapFunction
        (renderCostMatrix gapGroup)
        (createUkkonenMethodMatrix coefficient alphabet)
        lhs
        rhs
    where
        ...
        --     OR
        -- > coefficient == 0
        noGainFromUkkonenMethod = or [lesserLen <= 4, 2 * longerLen >= 3 * lesserLen, coefficient == 0]
            where
...
{-# INLINABLE createUkkonenMethodMatrix #-}
...
createUkkonenMethodMatrix minimumIndelCost alphabet overlapFunction longerTop lesserLeft = U
    $ ukkonenUntilOptimal startOffset
    where
    -- General values that need to be in scope for the recursive computations.
        ...
        ukkonenUntilOptimal inOffset
            ...
            where
                offset
                    | quasiDiagonalWidth + inOffset >= lesserLen = maximumOffset
                    | otherwise                                  = inOffset

                ukkonenMatrix = Ribbon.generate rows cols generatingFunction $ toEnum offset

                generatingFunction =
                    needlemanWunschDefinition gapGroup overlapFunction longerTop lesserLeft ukkonenMatrix
