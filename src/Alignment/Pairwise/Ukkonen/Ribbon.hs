...
generate x y f alpha = result
    where
        result      = Ribbon { height = h, width = w, diagonal = d, offset = a, linear = vector }
        ...
        points      = [ (i, j) | i <- [0 .. (h - 1)], j <- [max (i - a) 0 .. min (i + d + a - 1) (w - 1)] ]
...
ribbonLookup (i, j) r
    ...
    where
        ...
        outsideBounds = or [i < 0 || h <= i, j < 0 || w <= j, x > upperBarrier, y > lowerBarrier]
