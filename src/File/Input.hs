...
import Data.Vector.Unboxed.NonEmpty qualified as VU
import Data.Void
...
parseFileInput :: UserInput -> IO (Either String FileInput)
...
runFileParsers
    :: UserInput -> IO (CPUTime, Validation (NonEmpty String) (FastaParseResult, BTree () (), TCM))
runFileParsers input = timeOp $ do
    ...
    where
        readAndParse :: Parsec Void String a -> FilePath -> IO (Validation (NonEmpty String) a)
...
runUnification
    ...
    -> m
           ( CPUTime
           , Validation (NonEmpty String) (Alphabet Char, Matrix Word, BTree b PreliminaryNode)
           )
...
validateSymbolsAndAlphabet (TCM symbolList _) m = fromEither $ case foldMapWithKey f m of
    []     -> Right ()
    x : xs -> Left $ x :| xs
    where
        symbolSet :: Set Char
        ...
        f i s = case catMaybes . toList $ mapWithKey g s of
            []     -> []
            ...
            where
                preamble =
                    "For leaf "
                        <> unpack i
                        <> ", the following symbols were found but not specified in the alphabet: "

                g :: Int -> VU.Vector Char -> Maybe String
                g k v = case filter (`notElem` symbolSet) $ VU.toList v of
                    []     -> Nothing
                    x : xs -> Just $ fold
                        [ "  at index "
                        , show k
...
unifyInput
    :: (Foldable1 f, Key c ~ Text, Keyed c, Lookup c, Traversable c)
    => Alphabet Char
    -> c (f (VU.Vector Char))
    -> BTree b a
    ...
    where
        ...
        initializedTree = traverse f leafTaggedTree
            where
                ...
                f k = validationNel $ case k `lookup` dataCollection of
                    Nothing -> Left . LeafLabelMissingInDataSet $ unpack k
                    Just xs -> let !ss = buildSymbolString xs in Right $ PreliminaryNode 0 0 ss
                    where
                        buildSymbolString = foldMap1 (pure . buildAmbiguityGroup)
                        buildAmbiguityGroup x =
                            let !y = encodeAmbiguityGroup alphabet $ VU.toNonEmpty x in Align y y y
