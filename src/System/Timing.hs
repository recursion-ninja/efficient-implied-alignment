...
instance Show CPUTime where
    ...
    show (CPUTime x)
        ...
        | x < mSecond = let (q, r) = x `quotRem` μSecond
                        in  fold [show q, ".", zeroPad 3 (r `div` nSecond), "μs"]
        | x < second = let (q, r) = x `quotRem` mSecond
                       in  fold [show q, ".", zeroPad 3 (r `div` μSecond), "ms"]
        | x < minute = let (q, r) = x `quotRem` second
                       in  fold [show q, ".", zeroPad 3 (r `div` mSecond), "s "]
...
zeroPad k i = replicate (k - length shown) '0' <> shown where shown = show i
