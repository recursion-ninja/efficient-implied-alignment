...
type instance Key (BTree _b) = Text
...
instance Bifoldable BTree where
    ...
    bifoldr f g a (Internal (NodeDatum _ x) lhs rhs) = f x $ bifoldr f g (bifoldr f g a lhs) rhs
...
instance Bitraversable BTree where
    ...
    bitraverse f g (Internal (NodeDatum i x) lhs rhs) =
        Internal <$> (NodeDatum i <$> f x) <*> bitraverse f g lhs <*> bitraverse f g rhs
...
instance Applicative NodeDatum where
    ...
    pure x = NodeDatum { identifier = mempty, nodeDatum = x }

    liftA2 f (NodeDatum _ x) (NodeDatum s y) = NodeDatum s $ f x y


treeZipWith :: (b -> a) -> (a -> a -> c) -> BTree b a -> BTree b a -> BTree c c
treeZipWith f g (Internal x lhs1 rhs1) (Internal y lhs2 rhs2) =
    Internal (liftA2 g (f <$> x) (f <$> y)) (treeZipWith f g lhs1 rhs1) $ treeZipWith f g lhs2 rhs2
...
preorder
    ...
    -> BTree a a
    -> BTree b c
preorder rootTransformation internalTransformation leafTransformation rootNode = case rootNode of
    Leaf{} ->
        error
            $  "Cannot perform pre order on single node tree.\n"
    ...
    where
        go parentNode isLeft currentNode = case currentNode of
...
renderPhylogeny :: (a -> Text -> String) -> BTree b a -> String
renderPhylogeny f = horizontalRendering . toBinaryRenderingTree (const (const "")) f


renderAlignment :: (b -> Text -> String) -> (a -> Text -> String) -> BTree b a -> String
renderAlignment f g = horizontalRendering . toBinaryRenderingTree f g


toBinaryRenderingTree :: (b -> Text -> String) -> (a -> Text -> String) -> BTree b a -> BinaryRenderingTree
toBinaryRenderingTree f g tree = case tree of
    ...
    where
        ...
        applyAtCenter center upper lower (x :| xs) = (upper <> x) :| snd (foldr f (False, []) xs)
            where
                ...
                f str (crossedMidPoint, acc) = case str of
                    h : _ | not crossedMidPoint && h `notElem` "└┌│├ " -> (True, (center <> str) : acc)
                    _
                        | crossedMidPoint -> (True, (upper <> str) : acc)
