...
module Data.Matrix.ZeroIndexed
    ( Stupid.Matrix ()
    , Stupid.forceMatrix
    , Stupid.ncols
    , Stupid.nrows
...
getElem
    ...
    -> Matrix a -- ^ Matrix
    -> a
getElem i j mtx = case errorMessage of
    Just err -> error err
    ...
    where
        ...
        n           = Stupid.ncols mtx
        errorPrefix = fold
            [ "The call to Matrix indexing at point ("
            , show i
            , ","
            , show j
            , ") is malformed for the "
            , show m
            , "x"
            , show n
            , " matrix,"
            ]
        errorNegRowCount = if i < 0
            then Just $ fold ["the row index ", show i, ", is less than the lower bound 0"]
            else Nothing
        errorNegColCount = if j < 0
            then Just $ fold ["the column index ", show j, ", is less than the lower bound 0"]
            else Nothing
        errorBigRowCount = if i >= m
            then Just $ fold ["the row index ", show i, ", is greater than the upper bound ", show m]
            else Nothing
        errorBigColCount = if j >= n
            then Just $ fold ["the column index ", show j, ", is greater than the upper bound ", show n]
            else Nothing
        errorMessage =
